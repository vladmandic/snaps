...
var require_jquery = __commonJS({
  "node_modules/.pnpm/jquery@3.6.1/node_modules/jquery/dist/jquery.js"(exports, module) {
    (function(global, factory) {
      ...
      var swap = function(elem, options, callback2) {
        var ret, name, old = {};
        for (name in options) {
...
function renderCalendar(year) {
  ...
  (0, import_jquery.default)("#calendar-title").text(year);
  (0, import_jquery.default)("#prevyear").attr("href", "#" + (year - 1)).text(year - 1);
  (0, import_jquery.default)("#nextyear").attr("href", "#" + (year + 1)).text(year + 1);
  (0, import_jquery.default)("#prevyear").off("click");
  (0, import_jquery.default)("#nextyear").off("click");
  (0, import_jquery.default)("#prevyear").on("click", () => renderCalendar(year - 1));
  (0, import_jquery.default)("#nextyear").on("click", () => renderCalendar(year + 1));
  (0, import_jquery.default)("#calendar-months").html('<tr id="monthnames"></tr>');
  (0, import_jquery.default)("#calendar-days").html('<div class="day mo"></div>');
  for (let day = 0; day < 37; day++) {
    const weekday = ["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"][day % 7];
    (0, import_jquery.default)("#calendar-days").append(`<div class="day">${weekday}</div>`);
  }
  for (let month = 1; month <= 12; month++) {
    const monthName = new Date(year, month, 0).toLocaleString("en-US", { month: "short" });
    (0, import_jquery.default)("#monthnames").append(`<td class="mc mo" valign="top"><span id="month${month}">${monthName}</span></td>`);
    const monthId = "#month" + month;
    let template = `<div class="months"><div class="mo">${monthName}</div>`;
    for (let i = 0; i < 37; i++)
      template += '<div class="dt"></div>';
    template += "</div>";
    (0, import_jquery.default)(monthId).html(template);
    const dt = (0, import_jquery.default)(monthId + " div.dt");
    let index = new Date(year, month - 1, 1).getDay();
    const max = new Date(year, month, 0).getDate();
    for (let day = 1; day <= max; day++)
      dt.eq(index++).attr("id", `dt-${new Date(year, month - 1, day).toISOString().slice(0, 10)}`).html(`${day}`);
  }
  const today = new Date().toISOString().slice(0, 10);
  (0, import_jquery.default)(`#dt-${today}`).css("background", "gray");
  (0, import_jquery.default)(`#dt-${today}`).css("font-weight", "bold");
  (0, import_jquery.default)("calendar").show();
}
function addNote(date, note, callback2) {
  const dt = date.toISOString().slice(0, 10);
  const el = (0, import_jquery.default)(`#dt-${dt}`);
...
async function start() {
  ...
  renderCalendar();
  for (const [dt, images] of Object.entries(dates))
    addNote(new Date(dt), `${images.length} images`, callback);
}
window.onload = start;
